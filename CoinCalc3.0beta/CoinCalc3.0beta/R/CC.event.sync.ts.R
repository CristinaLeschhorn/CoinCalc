...
# CC.event.sync.ts performes event synchronisation with local delT including a shuffle significance tests using time series and some parameters.

CC.event.sync.ts<-function(a,b,max_delT=min(length(a),length(b)),tau=0,sigtest=TRUE,reps=1000,alpha=0.05){
  if (length(table(a)) > 2) {
    print("|-------------    ERROR #1    -------------|")
