...
# CC.eca.es performes event coincidence analysis including five different significance tests using event time series and some parameters.

CC.eca.es <- function(seriesA,seriesB,spanA,spanB,delT=0,alpha=0.05,sym=FALSE,tau=0,sigtest="poisson",reps=1000,period=0){ 
  ...
  if(sigtest=="poisson"){
    ...
    if(sym==FALSE){
      ...
      for(Ktmp in K_prec:(N_A-N_A_edge)){
        Ptmp=choose((N_A-N_A_edge),Ktmp) * ( (1-((1-(delT/(Tlen-tau)))**N_B)) **Ktmp) * (((1-(delT/(Tlen-tau)))**N_B)**(N_A-N_A_edge-Ktmp))
        Pprec=Pprec+Ptmp}  # end for Ktmp
      # Calculation for Trigger Coincidence 
      Ptrigg=0
      for(Ktmp in K_trigg:(N_B-N_B_edge)){
        Ptmp=choose((N_B-N_B_edge),Ktmp) * ( (1-((1-(delT/(Tlen-tau)))**N_A)) **Ktmp) * (((1-(delT/(Tlen-tau)))**N_A)**(N_B-N_B_edge-Ktmp))
  ...
  if (sigtest == "time.bootstrap") {
    Tlen=length(span)
    surdist = matrix(NA, 2, reps)
    min_time=min(which(CC.ecr.es(seriesA,spanA,offset=Tlen/10,siglevel=FALSE)[2][[1]]<1/exp(1))[1],which(CC.ecr.es(seriesB,spanB,offset=Tlen/10,siglevel=FALSE)[2][[1]]<1/exp(1))[1])
    max_time=Tlen/10
    if(period==0){
      period=min_time
    }else{
    ...
    for (surno in 1:reps) {
      ...
      num=0
      for(i in suA){
        surA=c(surA,seriesA[seriesA>(i*period-period) & seriesA<=(i*period)]-(i-1)*period+num*period)
        surB=c(surB,seriesB[seriesB>(i*period-period) & seriesB<=(i*period)]-(i-1)*period+num*period)
        num=num+1
      }
      K_prec_sur = 0
      for (step_a in (1+N_A_edge):N_A) {
  ...
  if (sigtest == "event.bootstrap") {
    max_time=min(length(seriesA),length(seriesB))/10
    waitA<-(c(seriesA,0)-c(0,seriesA))[2:(length(seriesA)-1)]
    waitB<-(c(seriesB,0)-c(0,seriesB))[2:(length(seriesB)-1)]
    min_time=max_time
    for(i in 0:max_time){
    ...
    surdist = matrix(NA, 2, reps)
    for (surno in 1:reps) {
      cutsA=c(0,seriesA[c(1:(length(seriesA)/period))*period],length(seriesA))
      cutsB=c(0,seriesB[c(1:(length(seriesB)/period))*period],length(seriesB))
      suA=sample(length(cutsA)-1)
      suB=sample(length(cutsB)-1)
      surA=c()
      surB=c()
      for(i in 1:length(suA)){
        surA=c(surA,seriesA[((suA[i]-1)*period):(suA[i]*period-1)])
      }
      for(i in 1:length(suB)){
        surB=c(surB,seriesB[((suB[i]-1)*period):(suB[i]*period-1)])
      }
      K_prec_sur = 0
      for (step_a in (1+N_A_edge):N_A) {
        if (sym == FALSE) {
